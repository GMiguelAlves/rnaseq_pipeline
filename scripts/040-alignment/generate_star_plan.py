#!/usr/bin/env python3
"""
Create a sample-level STAR quantification plan from a step-030 qc_plan.

Input qc_plan has one row per technical run. STAR should run once per
...
def build_star_plan(qc_plan: pd.DataFrame, project: str, output_root) -> pd.DataFrame:
    ...
    sample_rows["star_dir"] = sample_rows["sample_id"].apply(
        lambda sample: str(output_root / project / sample)
    )
    sample_rows["counts_file"] = sample_rows["star_dir"].apply(
        lambda path: str(as_pipeline_path(path) / "ReadsPerGene.out.tab")
    )
    sample_rows["bam_file"] = sample_rows["star_dir"].apply(
        lambda path: str(as_pipeline_path(path) / "Aligned.sortedByCoord.out.bam")
    )
    sample_rows["log_file"] = sample_rows["star_dir"].apply(
        lambda path: str(as_pipeline_path(path) / "Log.final.out")
    )
...
def main() -> None:
    ...
    parser.add_argument("--output-root", default=os.environ.get("STAR_QUANT_DIR", "star_quant"))
    parser.add_argument("--output", required=True, type=Path)
    parser.add_argument("--allow-missing", action="store_true")
    args = parser.parse_args()

    qc_plan = load_plan(args.qc_plan)
    star_plan = build_star_plan(qc_plan, args.project, as_pipeline_path(args.output_root))
    validate_plan(star_plan, args.allow_missing)

    args.output.parent.mkdir(parents=True, exist_ok=True)
    star_plan.to_csv(args.output, index=False)

    print(f"Wrote STAR plan: {args.output}")
    print(f"Samples: {len(star_plan)}")
